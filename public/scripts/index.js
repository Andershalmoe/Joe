...
async function pingServer() {
  const url = '/ping'; // URL til serverens ping endpoint
  const startTime = Date.now(); // Start tid
  console.log(`Pinger: Sending request to ${url}`);

  try {
    const response = await fetch(url); // Send anmodning
    ...
    console.log(`Response time: ${responseTimeHeader}`);
    console.log(`RTT: ${rtt} ms`);

    // Vis resultater i DOM
    const responseTimeParagraph = document.getElementById('responseTimeValue');
    responseTimeParagraph.textContent = 
      `Response Time: ${responseTimeHeader}, RTT: ${rtt} ms`;
  } catch (error) {
...
// Kald pingServer, når knappen trykkes
if (pingButton) {
